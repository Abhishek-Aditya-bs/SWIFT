...
class SwinTransformerBlockV2(nn.Module):
    ...
    def __init__(self, 
        ...
        self.attn = WindowAttentionV2(
            ...
            qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, attn_scale=attn_scale)
    ...
    # @torch.jit.ignore()
    def calculate_mask(self, x_size: Tuple[int, int], dtype: Optional[torch.dtype]=None, device: Optional[torch.device]=None) -> torch.Tensor:
        if self.shift_size > 0:
    ...
    def forward(self, x: torch.Tensor, x_size: Tuple[int, int]) -> torch.Tensor:
        ...
        if self.input_resolution == x_size:
            ...
            attn_windows = self.attn(x_windows, mask=self.calculate_mask(x_size, x_windows.dtype ,x_windows.device))
        ...
        if self.feat_scale:
            x_attn = x
            x_d, x_h = self.freq_decompose(x_attn)
            x_d = x_d * self.lamb1
            x_h = x_h * self.lamb2
            x_attn = x_attn + x_d + x_h
            x = shortcut + self.drop_path(self.norm1(x_attn + x_d + x_h))
        else:
